...
##################################
# Here we apply ±30° random rotations.
train_transform = transforms.Compose([
    transforms.RandomRotation(30),  # random ±30° rotation
    transforms.ToTensor(),
...
class ImprovedCNN(nn.Module):
    ...
    def forward(self, x):
        # Convolution + BN + ReLU + MaxPool #1
        x = F.relu(self.bn1(self.conv1(x)))
        x = F.max_pool2d(x, 2)

        # Convolution + BN + ReLU + MaxPool #2
        x = F.relu(self.bn2(self.conv2(x)))
        x = F.max_pool2d(x, 2)

        # Additional convolution + BN + ReLU + MaxPool #3
